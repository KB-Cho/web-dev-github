...
랜딩에서 정책서 만들기 선택
→ /new에서 기능 설명 입력
→ 로그인 입력 감지 시 /projects/login-policy로 이동
→ 질문은 한 화면에 하나씩 순차 답변
→ 마지막 질문 후 /documents/login-policy에서 답변 반영 초안 확인
```
...
## OpenAI API 정책 분석 Route

`POST /api/policy/analyze`는 기능 설명을 OpenAI API로 분석해 정책 도메인, 의사결정 질문, 정책서 목차, 가정을 JSON으로 반환합니다.
...
응답은 서버에서 지정한 JSON Schema를 따르도록 OpenAI Structured Outputs를 사용합니다.

- `featureSummary`: 기능 설명 요약
- `detectedDomains`: 감지된 정책 도메인과 이유, 신뢰도
- `questions`: 정책서 작성 전에 결정해야 하는 질문 목록
- `draftOutline`: 정책서 초안 목차
- `assumptions`: 입력만으로 단정할 수 없는 가정과 위험도

모르는 내용은 단정하지 않고 `assumptions` 또는 `questions`에 포함하도록 프롬프트에 명시했습니다.

### 환경변수

로컬 또는 Vercel 환경변수에 아래 값을 설정해야 합니다.

```bash
OPENAI_API_KEY=sk-...
```

모델을 바꾸고 싶다면 선택적으로 아래 환경변수를 설정할 수 있습니다.

```bash
OPENAI_POLICY_ANALYZE_MODEL=gpt-5.2
```

환경변수가 없으면 API route는 `OPENAI_API_KEY 환경변수가 설정되어 있지 않습니다.` 오류를 반환합니다.
